...
The `a11y-audit` agent owns phase numbering. During a phased site audit, this skill runs only in **Phase 3 (Keyboard)** after Phase 2 has a terminal status. Complete one pass: apply the checklist, return findings, and stop. Do not start another phase.
...
- Focus indicators (coordinate with a11y-contrast for visibility)
