...
The `a11y-audit` agent owns phase numbering. During phased site audits, load this skill in **Phase 0 (Discovery)** for stack detection and consult it during later phases for framework-correct fix examples. Review mode and standalone Phase 12 fixes may load it when the stack is known. It does not run tests and does not advance the phase gate.
