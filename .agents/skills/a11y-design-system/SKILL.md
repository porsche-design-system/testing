...
The `a11y-audit` agent owns phase numbering. During a phased site audit, this skill runs only in **Phase 5 (Visual)** when tokens or themes are confirmed or detected. Complete one pass: apply the checklist, return findings, and stop. Do not start another phase.
...
## Step 1: Identify Design System and Scope
...
## Step 3: Focus Ring Token Validation (WCAG 2.4.13)
...
## Step 4: Spacing Token Analysis (Touch Targets)
...
- **Touch target validation** — apply `a11y-playwright` (`--mode viewport`) to measure rendered target sizes across widths
- **WCAG criterion questions** — Read `a11y-severity-scoring` → [`references/wcag-guide.md`](../../../apm_modules/porsche-design-system/skills/packages/web-accessibility-audit/.apm/skills/a11y-severity-scoring/references/wcag-guide.md)
