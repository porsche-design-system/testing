...
The `a11y-audit` agent owns phase numbering. During a phased site audit, this skill runs only in **Phase 2 (Structure)** after Phase 1 has a terminal status. Complete one pass: apply the checklist, return findings, and stop. Do not start another phase.
...
This skill is a checklist module for `a11y-audit` covering non-visual text quality. Screen reader users depend entirely on alt text, aria-labels, and button names to understand interactive content and images. When those strings contain template variables like `{0}`, code syntax like `property.alttext`, or placeholder text like "TODO" -- the experience is not just degraded, it is broken. Ensure that every non-visual text string on a page communicates meaningful, human-readable content.
...
- Whether alt text is structurally present (that is a11y-alt-text-headings)
- Whether ARIA attributes are syntactically valid (that is a11y-aria)
- Whether link text is ambiguous like "click here" (that is a11y-links)
- Whether form labels are programmatically associated (that is a11y-forms)
...
Detects accessible names that are present but contain no meaningful content. This is different from a missing `alt` attribute (caught by a11y-alt-text-headings). These have the attribute but it contains only whitespace, invisible characters, or zero-width spaces.
...
3. Do any accessible names consist only of a dotted identifier (`property.name`)?
4. Are there any accessible names that look like variable names rather than descriptions?

### Placeholder and Test Text

5. Are there accessible names whose entire text is a placeholder token (TODO, FIXME, TBD, test, foo, bar)?
...
Return catalog-valid finding objects to `a11y-audit`; do not write a shared batch file. The orchestrator writes immutable `$SCRATCH/findings-agent-phase-<N>-page-<M>.json` batches. **May emit:** `accessible-name-quality`, `alt-text-quality`. Map template syntax, an entire placeholder token, or a dotted identifier to `accessible-name-quality`; generic or filename alt to `alt-text-quality`. Do not invent TQR-* ids. Do not treat brand camelCase such as `iPhone` as a failure.

Skip findings whose accessible name is inside a code editor, code sample, or developer-tools UI.
